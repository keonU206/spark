...
> ⚠️ **시안에 친구 추가 화면이 없습니다.**
> "친구의 운동 현황"은 있는데 친구를 어떻게 맺는지가 정의돼 있지 않습니다.
> 모임 멤버가 곧 친구인지, 별도 친구 기능인지 **기획 확인이 필요합니다.**
...
| # | 항목 |
|---|------|
| 1 | **친구 관계를 어떻게 맺는가** — 시안에 친구 추가 화면이 없습니다 |
| 2 | **세션 시작 API** — 중단 통계를 잡으려면 필요합니다 |
| 3 | **설문 선택지** — enum 고정인지, 코드값+표시명 분리인지 |
| 4 | **자세 데이터 저장 여부** — 시안은 "즉시 삭제"인데 철회 시 "영구 삭제" 문구가 있어 모순 |
| 5 | **모임 권한** — 방장/멤버 구분이 필요한지 |
| 6 | **넛지 쿨다운** — 스팸 방지 정책 |
| 7 | **이미지 업로드** — 프로필·피드 사진. presigned URL 권장 |
